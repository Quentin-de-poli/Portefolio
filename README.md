# Salut ! Moi c'est Quentin

### **Développeur débutant de jeux vidéo passionné par la création de nouvelles experiences.**

Ce que j'aime dans cette création c'est transformer mes idées (complexes ou pas) en code et d'essayer de nouvelles mechaniques. Quand je ne code pas je joue au jeux vidéo, je sort avec des potes, je regarde des film/séries. et souvent c'est ce qui m'inspire a vouloir créer, soit depuis rien, soit en mélangeant l'univert du jeux vidéo et mon experience.

---

##  Ma Boîte à Outils 

| Secteur | Technologies |
| --- | --- |
| **game dev** | Godot, GDScript |
| **Logic & Scripting** | Python |
| **Développement Web** | HTML5, CSS3, JavaScript, JSON |
|**Outils & Pipeline**| Git, VS Code|

---

##  Ma formation

Je n'ai pas encore de projet de jeux, je me forme encore, mais a la place des mes prjet je vous propose de voir ma progression :

je me forme avec cette playlist [![Godot University](https://i.ytimg.com/vi/bP9zQyoYP28/hqdefault.jpg?sqp=-oaymwEcCPYBEIoBSFXyq4qpAw4IARUAAIhCGAFwAcABBg==&rs=AOn4CLBPpA8D9v5s6VraaKPAeMVhttN2Mw)](https://youtube.com/playlist?list=PLrT2fbyJrAIctd7zNUsdPakIllX2lhrzo&si=HwFNG5ysS0ujQHSH)


> *Une description courte et percutante (1 phrase) sur le problème que tu as résolu.*

* **Tech stack :** React, Firebase, Stripe API.
* **Le petit plus :** Implémentation d'un système de paiement sécurisé en moins de 48h.
* **[Voir le Code](https://www.google.com/search?q=https://github.com/ton-profil/projet-1)** | **[Démo Live](https://www.google.com/search?q=https://ton-lien-demo.com)**

###  [Nom du Projet 2]

> *Description de l'utilité du projet. Pourquoi l'as-tu construit ?*

* **Tech stack :** Python, Pandas, Scikit-learn.
* **Résultat :** Amélioration de la précision des prédictions de 15%.
* **[Voir le Code](https://www.google.com/search?q=https://github.com/ton-profil/projet-2)**

###  [Nom du Projet 3]

> *Application mobile de gestion de [sujet].*

* **Tech stack :** React Native, Redux, Node.js.
* **[Voir le Code](https://www.google.com/search?q=https://github.com/ton-profil/projet-3)**

---

##  Statistiques GitHub

---

##  Me contacter

* **LinkedIn :** [linkedin.com/in/ton-profil](https://www.google.com/search?q=https://linkedin.com/in/ton-profil)
* **Portfolio :** [ton-site-perso.com](https://www.google.com/search?q=https://ton-site-perso.com)
* **Email :** [ton-email@exemple.com]

*"Le code est comme l'humour. Quand il faut l'expliquer, c'est qu'il est mauvais."* — Cory House

---

###  Conseils pour personnaliser ce modèle :

1. **Remplace les liens :** Change `TON_PSEUDO` par ton véritable identifiant GitHub dans la section statistiques pour que les graphiques s'affichent.
2. **Sois spécifique :** Dans la description des projets, ne dis pas juste "J'ai fait une app", dis "J'ai optimisé les temps de chargement de 30% grâce à...".
3. **Les émojis :** Utilise-les avec parcimonie pour structurer la lecture, mais n'en abuse pas pour rester professionnel.

Souhaites-tu que je t'aide à rédiger une description plus spécifique pour l'un de tes projets en particulier ?
